...
export default function LoginPage() {
  ...
  async function handleQuickLogin(email: string, role: string, callbackUrl: string) {
    setLoadingRole(role)
    try {
      ...
      <div className="max-w-4xl w-full">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-3xl border border-blue-100 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col justify-between group transform hover:-translate-y-1">
            ...
            <div className="p-8 bg-slate-50 border-t border-gray-100 space-y-3">
              <button
                onClick={() => handleQuickLogin('student@school.ua', 'student', '/')}
                disabled={loadingRole !== null}
                className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm transition-all shadow-md shadow-blue-200 hover:shadow-lg disabled:opacity-50"
              >
                {loadingRole === 'student' ? 'Входимо...' : 'Швидкий вхід як Учень'}
              </button>

              <button
                onClick={() => signIn('google', { callbackUrl: '/' })}
                disabled={loadingRole !== null}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors font-semibold text-gray-700 text-sm disabled:opacity-50"
              >
          ...
          <div className="bg-white rounded-3xl border border-emerald-100 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col justify-between group transform hover:-translate-y-1">
            ...
            <div className="p-8 bg-slate-50 border-t border-gray-100 space-y-3">
              <button
                onClick={() => handleQuickLogin('teacher@school.ua', 'teacher', '/teacher')}
                disabled={loadingRole !== null}
                className="w-full py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm transition-all shadow-md shadow-emerald-200 hover:shadow-lg disabled:opacity-50"
              >
                {loadingRole === 'teacher' ? 'Входимо...' : 'Швидкий вхід як Вчитель'}
              </button>
